     ...
     if (i < input_cols - kernel_cols+1 && j < input_rows - kernel_rows+1) {
         ...
         for (int k=0;k<kernel_rows; ++k) {
             for (int l=0;l<kernel_cols; ++l) {
                 res += input[((j+k)*input_cols+i+l)]*kernel[k*kernel_cols+l];
             }
         }
...
extern "C" void solve(const float* input, const float* kernel, float* output, int input_rows,
    int input_cols, int kernel_rows, int kernel_cols) {
     ...
     convolution_2d_kernel<<<gridDim, blockDim>>>(input, kernel, output, input_rows, input_cols, kernel_rows, kernel_cols);
